...
export const CHANGELOG: LogEntry[] = [
  {
    ...
    items: [
      'The game can now wear hand-made 3D bus models. Drop .glb files into public/models with a small manifest and any bus model swaps from the built-in mesh to the real one — company and line colors are painted on by material name, the station viewer opens the model\u2019s own door, and anything missing or broken falls back to the built-in mesh. Authoring guide in docs/MODELS.md.',
      ...
      'Demand is drawn as a scatter now: each dot stands for a few hundred people, sprinkled across the area they live or work in, so up close you can see a corridor thin out street by street. Pull back and neighbouring dots merge into the single blobs the map has always shown. District names shrink as you zoom out instead of shouting at you across a city-wide view, and depot and landmark markers shrink with them.',
      'Visitors no longer commute. Everyone else in the city runs start point to end point — out to work in the morning, back home at night — but tourists were modelled as extra jobs, which had them arriving at nine and leaving at five. They now travel their own way: shorter hops from one sight to the next, back into the hotel quarter through the evening, and a steady pull toward the airport and the station by people leaving town. They ride at their own hours too, so a line through the tourist quarter fills out the middle of the day instead of peaking twice.',
      'Three map layers instead of seven. Residents (where trips start), Destinations (where they end — jobs, campuses, hotels, the airport and the rail station together) and Travel modes. Tourism and education were always part of the jobs figure and airport and rail trips are destinations like any other, so six layers were mostly redrawing each other.',
      'Depot zoning now uses real land use. It used to guess from census figures — more jobs than residents at low density — which also describes a university, a hospital or a retail park, and that is how depots were being approved on residential streets. The game now reads the land actually zoned industrial (plus rail yards, ports and airfields) straight from the map data, baked into the city so it works offline. The green tint and the rule are the same list, so if it is green you can build there.',
      'Worcester has its train station back. Rail stations are tagged several different ways on the map, and the game only asked for one of them — Union Station is drawn as a building, so it was invisible to the game and never generated a single rail passenger.',
